...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
    ...
    protected function access($permissions) {
        $check = true;

        foreach ($permissions as $rule => $access) {
            if (!$this->acl->can($rule, $access)) {
                $check = false;
        ...
        if (!$check) {
            $this->response(array(
                ...
                'data'   => array(
                    'type'   => 'global',
                    'status' => 'error',
                    'text'   => sprintf('Недостаточно прав для выполнения данной операции (требуется значение <strong>%d</strong> для параметра <strong>%s</strong>).', $access, $rule),
                ),
                'reason' => sprintf('Permission denied. Rule must be at least %d for %s.', $access, $rule),
            ), 403);
        }
    }
	...
	public function __construct()
	{
        ...
        // $user = $this->session->userdata('user.logged');

        // if (!$user) {
        //     $this->response(array(
        //         'status' => false,
        //         'reason' => 'API available for logged-in users only.',
        //     ), 401);
        // } else {
        //     $this->acl->init($this->session->userdata('user.role'), true);
        // }
    ...
    protected function _parse_options()
    {
        ...
        {
            parse_str(file_get_contents('php://input'), $this->_options_args);
        }
    }
